...
switch_desktop_on_exit (Event *e)
{
    ...
    //vdesks.set_vdesktop (faces->get_current_column(), faces->get_current_row() );
...
render_scene(void)
{
    ...
    if (current_time - last_time > cfg->options->animation_speed) {
		//printf("render_scene - 1 -\n");
        if (ISSET_STATE(STATE_EXIT)) {
		//printf("render_scene - 11 -\n");
...
//#if defined DEBUG_LIGHTING
    ...
    if (cfg->options->show_digits) {
        ...
        glOrthof(0,GLWin.get_width(),0,GLWin.get_height(),-1.0,1.0);
	...
	eglSwapBuffers ( GLWin.egl_display, GLWin.egl_surface );  // get the rendered buffer to the screen

	//SDL_GLES_SwapBuffers( );
    //glXSwapBuffers(GLWin.get_display(), GLWin.get_window());	//gltogles
...
initialize_3ddesktop (void)
{
    ...
    GLWin.open_display();
	printf("entering create_GL_window()\n");
	// gltoglesjj
    GLWin.create_GL_window("Han Maokun",  cfg->options->do_fullscreen);
    ...
    if (cfg->options->acquire || cfg->mirror || 1) {
