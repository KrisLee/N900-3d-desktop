...
public:
    ...
    void render (void) {
        ...
        if (texture_id != (GLuint)-1) {
            ...
            //glColorMaterial(GL_FRONT_AND_BACK,GL_DIFFUSE);	// gltogles
            ...
            //glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);	// gltogles
            ...
            // if (wireframe == 1 || (wireframe == -1 && cfg->options->use_wireframe))	// gltogles
                //glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);	// gltogles
		...
		*/
			float tex[8] = {
			  ...
			  float ver[12] = {
				 topleft.v[0], topleft.v[1], topleft.v[2],
				 bottomleft.v[0], bottomleft.v[1], bottomleft.v[2],
				 topright.v[0], topright.v[1], topright.v[2],
				 bottomright.v[0], bottomright.v[1], bottomright.v[2]
				 /*
				         -1.0f, -1.0f,  1.0f,      
				         1.0f, -1.0f,  1.0f,      
				        -1.0f,  1.0f,  1.0f,      
    ...
    void load_texture_data (int texture_size, unsigned char *data) {
		...
		TextureImage = IMG_Load( "/opt/nehegles/screen0.jpg" );
		printf("I am here loading:/opt/nehegles/screen0.jpg\n");
*/
        if (texture_id == (GLuint)-1)
	    ...
	    //gluBuild2DMipmaps( GL_TEXTURE_2D, GL_RGB, TextureImage->w, TextureImage->h, GL_RGB, GL_UNSIGNED_BYTE, TextureImage->pixels );
